...
/* Include CMSIS-NN optimized kernels */
#include "tensorflow/lite/micro/kernels/cmsis_nn/cmsis_nn_utils.h"

#ifdef __cplusplus
}
#endif

LOG_MODULE_REGISTER(tflite_nrf, CONFIG_LOG_DEFAULT_LEVEL);
...
namespace {
  ...
  /* Smaller tensor arena size for nRF52840 due to memory constraints */
...
#define MODEL_SIZE (24 * 1024) /* Maximum expected model size */
...
int tflite_init(struct tflite_context *ctx)
{
    ...
    LOG_INF("Initializing TensorFlow Lite for nRF52840");

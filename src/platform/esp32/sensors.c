...
static int read_soil_moisture(float *value_out)
{
    ...
    float moisture = 100.0 - ((soil_sample_buf - SOIL_WET_VALUE) * 100.0) / (SOIL_DRY_VALUE - SOIL_WET_VALUE);
...
static int read_light_level(float *value_out)
{
    ...
    /* Lower ADC reading = more light */
    float light = 100.0 - ((float)light_sample_buf * 100.0) / (1 << ADC_RESOLUTION);
...
static int read_air_movement(float *value_out)
{
    ...
    /* Convert ADC value to relative air movement value */
    *value_out = (float)air_sample_buf;
